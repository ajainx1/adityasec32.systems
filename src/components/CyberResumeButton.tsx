...
import { Download, ShieldCheck, Lock, Terminal as TermIcon, FileCheck, KeyRound } from "lucide-react";
import ResumeSecurityChallengeModal from "./ResumeSecurityChallengeModal";
...
export default function CyberResumeButton({
  ...
  const handleMouseEnter = () => {
    setIsHovered(true);
    if (!isDownloading) {
      startScramble(variant === "nav" ? "UNLOCK_CV.PDF" : "ADITYA_JAIN_CV.PDF");
    }
  };
  ...
  const handleChallengeSuccess = () => {
    ...
    if (intervalRef.current) clearInterval(intervalRef.current);
    setDisplayText("✓ DECRYPTED & DOWNLOADED");
    ...
    <>
      ...
      {variant === "nav" && (
        <button
          ...
          onClick={handleClick}
          className={`group relative overflow-hidden px-4 sm:px-5 py-2 text-xs font-bold font-mono rounded-full bg-emerald-500 text-slate-950 hover:bg-emerald-400 transition-all flex items-center gap-2 shadow-lg shadow-emerald-500/20 hover:shadow-emerald-500/40 min-h-[44px] border border-emerald-400/50 cursor-pointer active:scale-95 ${className}`}
          title="Unlock Verified Resume PDF (SecOps Decryption Gateway)"
        >
      ...
      {variant === "bottom" && (
        <div className="inline-flex flex-col items-center gap-2">
          <button
            ...
            onMouseLeave={handleMouseLeave}
            onClick={handleClick}
            className={`group relative overflow-hidden px-8 py-3.5 rounded-full bg-emerald-500 text-slate-950 font-mono font-bold text-xs hover:bg-emerald-400 transition-all shadow-lg shadow-emerald-500/25 hover:shadow-emerald-500/40 min-h-[44px] flex items-center gap-2.5 border border-emerald-300/40 cursor-pointer active:scale-95 ${className}`}
          >
            <span className="absolute inset-0 -translate-x-full group-hover:translate-x-full transition-transform duration-700 bg-gradient-to-r from-transparent via-white/40 to-transparent pointer-events-none" />
            {isDownloading ? (
              <ShieldCheck className="w-4 h-4 text-slate-950 animate-bounce" />
            ) : (
              <Lock className="w-4 h-4 group-hover:text-slate-950 transition-colors" />
            )}
            <span className="relative z-10">{displayText}</span>
          </button>
        </div>
        ...
        <div className="inline-flex flex-col items-start gap-2">
          ...
          >
            ...
            {isDownloading ? (
              ...
              <Lock className="w-4 h-4 text-slate-950 group-hover:-translate-y-0.5 transition-transform" />
            )}
          ...
          {showChecksum && (
            <div className="flex items-center gap-2 text-[10px] font-mono text-emerald-400/90 pl-1">
              <span className="inline-flex items-center gap-1 bg-emerald-950/60 border border-emerald-500/30 px-2 py-0.5 rounded text-emerald-300">
                ...
                <span>Level 4 Decryption Gateway</span>
              </span>
